...
Use this API to create a product discount activity or a flash deal activity. You cannot create a coupon activity with this API. To do that, you'll need to go to the seller center or the seller app. To update the products or SKUs included in the activity, use **_Update Activity Product_**.

## Requirements

- A valid seller **Access Token**
- A valid **Shop Cipher** (encrypted shop identifier)
- Use `event.createActivity()` from the SDK

Official documentation: [TikTok Shop – Create Activity](https://partner.tiktokshop.com/docv2/page/create-activity-202309)
...
| activity_type | string | Type of discount activity. Example: `FIXED_PRICE` |
